      ...
      <div class="card shadow">
        <div class="card-header bg-transparent">
          <div class="row align-items-center">
            <div class="col">
              <h6 class="text-uppercase text-muted ls-1 mb-1">Ranking PVE</h6>
              <h2 class="mb-0">Updates</h2>
            </div>
            <div class="col text-right">
              <button type="button" id="bulkdelete" class="btn btn-danger">Bulk Delete</button>
              <a class="btn btn-primary" href="<?php echo base_url("admin/pve/form") ?>">Novo Update </a>
            </div>
          </div>
        </div>
        <div class="card-body">
          <div class="row">
            <div class="col-lg-6">
              <h6 class="heading-small text-muted">Ativar update PVE</h6>
            </div>
            <div class="col-lg-6 text-right mb-4">
              <label class="custom-toggle">
                <form id="Formconfig" method="post" action="<?php echo base_url("admin/configuracoes/updatepve") ?>">
                  <input type="hidden" name="<?php echo $this->security->get_csrf_token_name(); ?>" value="<?php echo $this->security->get_csrf_hash(); ?>">
                  <input type="checkbox" onChange="$('#Formconfig').submit()" name="usarupdatepve" <?php echo ($this->config->item("usarupdatepve") == 1 ? "checked" : '') ?>>
                  <span class="custom-toggle-slider rounded-circle"></span>
                </form>
              </label>
            </div>
            <div class="col-md-12">
              <form id="Form" method="post" action="<?php echo base_url("admin/pve/bulkdelete") ?>">
                <input type="hidden" name="<?php echo $this->security->get_csrf_token_name(); ?>" value="<?php echo $this->security->get_csrf_hash(); ?>">
                          ...
                          <td class="text-center">
                            <div class="dropdown">
                              ...
                              <div class="dropdown-menu dropdown-menu-right dropdown-menu-arrow">
                                ...
                                <a class="dropdown-item" href="<?php echo base_url("admin/pve/form/" . $update->id) ?>">Editar</i></a>
          ...
          <h4 class="mb-0">Ultimo Processamento: <?php echo $this->config->item("UltUpdatepve") ?></h4>
          <h4 class="mb-0">Tempo de processamento:  <?php echo $this->config->item("TempoGastopve") ?> segundos</h4>
        </div>
      </div>
    </div>
  </div>
      ...
      $(document).ready(function () {
          $(".btndeletar").click(function () {
              Swal.fire({
                  ...
                  if (result.value) {
                      window.location = "<?php echo base_url('admin/pve/deletar/') ?>" + this.getAttribute('data-id');
                  }
              });
          });
      });
